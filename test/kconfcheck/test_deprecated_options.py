...
TEST_FILES_LOCATION = os.path.join(CURRENT_PATH, "tyconfigs", "check_deprecated")


class TestDeprecatedDetection:
    @pytest.mark.parametrize(
        "defaults_file_name", ["tyconfig.defaults.with_deprecated", "tyconfig.defaults.without_deprecated"]
    )
    def test_check_deprecated_options(self, defaults_file_name, monkeypatch):
        ...
        rename_file_path = os.path.join(TEST_FILES_LOCATION, "tyconfig.rename")
        ...
        if defaults_file_name == "tyconfig.defaults.with_deprecated":
            assert not is_valid
        elif defaults_file_name == "tyconfig.defaults.without_deprecated":
            assert is_valid
        else:
