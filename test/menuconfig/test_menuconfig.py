...
TYCONFIGS_PATH = os.path.join(TEST_FILES_PATH, "tyconfigs", "test_needs_save")


@pytest.mark.parametrize("version", ["1", "2"], indirect=True)
class TestNeedsSave:
    """
    Checking cases in which the tyconfig file needs to be saved.
    Does not check the actual content of the tyconfig file (this is done in test_kconfiglib.py).

    Principle:
        1) Set the KCONFIG_CONFIG environment variable to point to the tyconfig file.
        2) Load the Kconfig file and the tyconfig file.
    ...
    def assert_and_print_actual(self, val: bool, kconfig: "Kconfig") -> None:
        ...
        try:
            ...
            print(
                f"menuconfig {'wants' if needs_save else 'does not want'} to save the tyconfig file even "
    ...
    def test_no_change(self) -> None:
        ...
        os.environ["KCONFIG_CONFIG"] = os.path.join(
            TYCONFIGS_PATH, "tyconfig.no_change"
        )
    ...
    def test_new_symbol_in_kconfig(self) -> None:
        # New symbol is added to the Kconfig file (or removed from tyconfig file).
        # In other words, there is a symbol in Kconfig which is not in tyconfig.
        os.environ["KCONFIG_CONFIG"] = os.path.join(TYCONFIGS_PATH, "tyconfig.change")
        kconfig = Kconfig(os.path.join(KCONFIGS_PATH, "Kconfig"))
        menuconfig(kconfig, headless=True)
    ...
    def test_reset_from_different_value(self) -> None:
        ...
        os.environ["KCONFIG_CONFIG"] = os.path.join(
            TYCONFIGS_PATH, "tyconfig.reset_from_different_value"
        )
        kconfig = Kconfig(os.path.join(KCONFIGS_PATH, "Kconfig"))
        menuconfig(kconfig, headless=True)
    ...
    def test_reset_from_same_value(self) -> None:
        ...
        os.environ["KCONFIG_CONFIG"] = os.path.join(
            TYCONFIGS_PATH, "tyconfig.reset_from_same_value"
        )
        kconfig = Kconfig(os.path.join(KCONFIGS_PATH, "Kconfig"))
        menuconfig(kconfig, headless=True)
        _restore_default(kconfig.syms["CREW"].nodes[0])
    ...
    def test_user_value_changed(self) -> None:
        ...
        os.environ["KCONFIG_CONFIG"] = os.path.join(
            TYCONFIGS_PATH, "tyconfig.user_value_changed"
        )
        kconfig = Kconfig(os.path.join(KCONFIGS_PATH, "Kconfig"))
        menuconfig(kconfig, headless=True)
    ...
    def test_default_value_in_kconfig_changed(self, defaults_policy: str) -> None:
        ...
        menuconfig(kconfig, headless=True)
        self.assert_and_print_actual(
            True if defaults_policy == "kconfig" else False, kconfig
        )
