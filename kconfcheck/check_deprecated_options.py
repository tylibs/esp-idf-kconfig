...
def _prepare_deprecated_options(
    ...
    # used as "startswith" pattern to match tyconfig.[ci|defaults] variants
    files_to_check_pattern = ("tyconfig.ci", "tyconfig.defaults")
    ...
    # tyconfig.rename files can be explicitly specified in the command line as well,
    # but they are not checked, only used as a source of deprecated options.
    for file in tuple(files):
        if "tyconfig.rename" in file:
            deprecated_options.update(_parse_path(file))
            files.remove(file)

    # Recursively search for tyconfig files in the specified directories
    # tyconfig.rename: used as a source of deprecated options
    # tyconfig.[ci|defaults]: added to list of files to be checked
    if includes:
